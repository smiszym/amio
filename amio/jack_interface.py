...
class JackInterface(Interface):
    ...
    def init(self, client_name: str) -> None:
        ...
        self.jack_interface = amio._core.create_jack_interface(client_name)
    ...
    def _process_messages_and_print_logs(self) -> None:
        ...
        while not should_stop:
            amio._core.iface_process_messages_on_python_queue(self.jack_interface)
    ...
    def _collect_and_print_logs(self) -> None:
        ...
        arr = bytearray(4096)
        amio._core.iface_get_logs(self.jack_interface, arr)
    ...
    def get_frame_rate(self) -> float:
        ...
        return amio._core.iface_get_frame_rate(self.jack_interface)

    def get_position(self) -> int:
        if self.jack_interface is None:
            raise ValueError("Operation on a closed AMIO interface")
        return amio._core.iface_get_position(self.jack_interface)

    def set_position(self, position: int) -> None:
        if self.jack_interface is None:
            raise ValueError("Operation on a closed AMIO interface")
        amio._core.iface_set_position(self.jack_interface, position)

    def is_transport_rolling(self) -> bool:
        if self.jack_interface is None:
            raise ValueError("Operation on a closed AMIO interface")
        return amio._core.iface_get_transport_rolling(self.jack_interface) != 0

    def set_transport_rolling(self, rolling: bool) -> None:
        if self.jack_interface is None:
            raise ValueError("Operation on a closed AMIO interface")
        amio._core.iface_set_transport_rolling(self.jack_interface, 1 if rolling else 0)
    ...
    def set_current_playspec(
        ...
        amio._core.iface_set_playspec(self.jack_interface)

    def close(self) -> None:
        with self.should_stop_lock:
            self.should_stop = True
        self.message_thread.join()
        amio._core.iface_close(self.jack_interface)
    ...
    def _get_next_input_chunk(self) -> Optional[InputAudioChunk]:
        input_chunk = amio._core.iface_get_input_chunk(self.jack_interface)
        if input_chunk is None:
            return None
